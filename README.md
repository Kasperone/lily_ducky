...
| 🔒 **Change default credentials** | The WiFi C2 defaults (`LillyC2` / `quackquack`) are public. Change them in `firmware/src/config.h` before any test that isn't fully air-gapped. |
...
LillyDucky emulates a USB keyboard and executes **DuckyScript** payloads (the language of
...
1. Connect to the AP: SSID `LillyC2` (change it! see safety box above)
